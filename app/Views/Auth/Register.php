<?php
/**
 * Register Page — Kantin UPB
 *
 * Aturan (sesuai permintaan tim):
 *   - Field pakai NPM (9 digit angka) + password + konfirmasi password.
 *     TIDAK pakai email.
 *   - NPM wajib tepat 9 digit (client-side: pattern + maxlength + minlength;
 *     server-side: regex /^[0-9]{9}$/ di controller Auth::processRegister).
 *
 * @var string $title
...
<head>
    ...
    <meta name="description" content="Daftar akun baru Kantin UPB — registrasi dengan NPM & password.">
    <title><?= esc($title ?? 'Daftar Akun — Kantin UPB') ?></title>
...
<div class="auth-wrap">
    <main class="auth-card">
        <header class="auth-card__header">
            ...
            <p class="auth-card__subtitle">Daftar untuk mulai menggunakan Kantin UPB</p>
        </header>
        ...
        <form method="POST" action="<?= site_url('/register') ?>" autocomplete="on" novalidate>
            <?= csrf_field() ?>
            ...
            <div class="form-group">
                ...
                <p class="form-text">NPM harus tepat 9 digit angka (0-9). NPM akan menjadi identitas login Anda.</p>
            </div>
            ...
            <div class="form-group">
                ...
                <p class="form-text">Min. <?= esc($passwordMinLength ?? 8) ?> karakter. Disarankan kombinasi huruf besar, huruf kecil, dan angka.</p>
            </div>

            <div class="form-group">
                <label class="form-group__label" for="password_confirm">Konfirmasi Password</label>
                <div class="form-group__input-wrap">
                    <i class="fas fa-key"></i>
                    <input
                        type="password"
                        ...
                        minlength="<?= esc($passwordMinLength ?? 8) ?>"
                    >
                    <button type="button" class="password-toggle" data-target="password_confirm" aria-label="Tampilkan password" aria-pressed="false">
                        <svg class="icon-eye" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path>
                            <circle cx="12" cy="12" r="3"></circle>
                        </svg>
